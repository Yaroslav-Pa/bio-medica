  ...
  return (
    <section className='fixed top-16 left-5 m-5 flex flex-col w-fit gap-5'>
      <div>
        ...
        <input
          className='border-2 rounded-lg px-3 py-1 border-stone-400'
          type='text'
          name='searchField'
          value={searchValue}
          onChange={(e) => {
      ...
      <div className={"flex flex-col gap-2 transition-opacity "+(searchValue!=''? ' opacity-0 invisible':' opacity-100 visible')}>
        ...
        <div className='flex flex-col gap-3'>
          {array && array.map((text) => <Link href={`#${text}`} scroll={true} key={text} className="relative text-xl w-fit block after:block after:content-[''] after:absolute after:left-0 after:h-[2px] after:bg-black after:w-full after:scale-x-0 after:hover:scale-x-100 after:transition after:duration-300 after:origin-left transition-all duration-300 px-5 pl-0 hover:pl-3">{text}</Link>)}
        </div>
      </div>
